...
def plot_model_accuracy(y_test : np.array, y_class : np.array) -> None:
    ...
    plt.savefig(settings.ROOT_PATH["IMAGES"].joinpath("{}_{}".format(settings.GRAPH_NAME , "kMean_acc")) , dpi=200)


def main():
    x_train, y_train  = load_training_data()
    x_train, y_train = shuffle_data(x_train , y_train)
    k_mean_model = KMeans(n_clusters=0).fit(x_train,  y_train)

    x_test, y_test = load_testing_data()

    y_pred = k_mean_model.predict(x_test)
